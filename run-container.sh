...
docker run -i -t -v ${BITCOIN_REPO}:/bitcoin -v ${QA_ASSETS_REPO}:/qa-assets -m 3g --cpus=3 --cap-add=SYS_PTRACE --security-opt seccomp=unconfined --security-opt apparmor=unconfined bitcoin-fuzzing bash
